...
def score_decks(new_decks, away_sample_df, all_cards, card_cnt, home_card_list):
    header = home_card_list.copy()
    ...
    unique_decks_df = get_elixr(
        pd.DataFrame(new_decks, columns=all_cards, dtype='int8'))

    print(f'Length of away sample: {len(away_sample_df)}')
    outcome_est = np.zeros([len(unique_decks_df), (2 + card_cnt)])
    print(f'Number of decks to sample: {len(unique_decks_df)}')
    max_i = len(unique_decks_df)
    max_decks_per_iter = len(away_sample_df)
    i = 0
    while (i * max_decks_per_iter) < len(unique_decks_df):
        ind = range((i * max_decks_per_iter), min(((i + 1) * max_decks_per_iter), max_i))
        home_card_df = unique_decks_df.loc[ind, home_card_list].values
        score_df = get_elixr_away(get_elixr(pd.DataFrame(np.concatenate(
            [np.repeat(home_card_df, len(away_sample_df), axis=0), np.tile(away_sample_df.values, (len(ind), 1))],
            axis=1), columns=all_cards, dtype='int8')))

        score_df['home_pb_lseason'] = 5700
        score_df.loc[:, 'net_pb_bseason'] = 0
        score_df.loc[:, 'net_exp_level'] = 0
        score_df.loc[:, 'net_level_gap'] = 0
        score_df = get_segment(score_df)
        score_df = get_segment_away(score_df)

        _, score_df = add_trophs(score_df, analysis_sel_cols)
        _, score_df = add_segments_home(score_df, analysis_sel_cols)
        _, score_df = add_segments_away(score_df, analysis_sel_cols)
        _, score_df = add_elixir_home(score_df, analysis_sel_cols)
        _, score_df = add_elixir_away(score_df, analysis_sel_cols)
        _, score_df = code_features(score_df, analysis_sel_cols)
        _, score_df = code_away_features(score_df, analysis_sel_cols)
        _, score_df = code_home_features(score_df, analysis_sel_cols)
        score_stats_df = score_df.loc[:, stats_sel_cols]
        score_df = score_df.loc[:, analysis_sel_cols]
        base_est = predict(score_df, score_stats_df, stats_sel_cols, lr_only=False)
        j = 0
        for row in ind:
            outcome_est[row, 0:card_cnt] = home_card_df[j]
            base_inds = range(j * len(away_sample_df), (j + 1) * len(away_sample_df))
            outcome_est[row, card_cnt] = np.average(base_est[base_inds])
